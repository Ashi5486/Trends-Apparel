...
const OperationList = () => {
  ...
  const { data: operationsData } = useFetch(`${base_url}/api/operations`);
  ...
  useEffect(() => {
    const filtered = operationList.filter(op =>
      op.operationName?.toLowerCase().includes(input.toLowerCase())
    );
    setFilteredOperation(filtered);
    setCurrentPage(1);
  }, [input, operationList]);

  const handleAddList = () => setIsOpen(prev => !prev);

  const indexOfLast = entriesPerPage === 'all' ? filteredOperation.length : currentPage * entriesPerPage;
  const indexOfFirst = entriesPerPage === 'all' ? 0 : indexOfLast - entriesPerPage;
  const currentOperations =
    entriesPerPage === 'all'
      ? filteredOperation
      : filteredOperation.slice(indexOfFirst, indexOfLast);

  const totalPages = Math.ceil(
    entriesPerPage === 'all' ? 1 : filteredOperation.length / entriesPerPage
  );
  ...
  return (
    <div className="flex flex-col gap-6 mt-2 bg-white rounded-lg">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl font-semibold text-gray-600">Operation List</h2>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          ...
          >
            <FaPlus className="inline mr-1" /> Add New Operation
          </button>
      ...
      {entriesPerPage !== 'all' && (
        <div className="flex flex-col sm:flex-row justify-between items-center px-3 py-3 text-l text-gray-400">
          <span>
            ...
            {Math.min(indexOfLast, filteredOperation.length)} of{' '}
            {filteredOperation.length} entries
          </span>
