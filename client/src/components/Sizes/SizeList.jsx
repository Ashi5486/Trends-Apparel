...
import SizeItem from './SizeItem'; // This component handles individual size display
import AddSize from './AddSize'; // This component is for adding a new size
...
const SizeList = () => {
  ...
  const { data, isLoading, error } = useFetch(`${base_url}/api/sizes`); // Change endpoint here

  useEffect(() => {
    if (data) {
      const sizes = data.sizeData || data; // Adjust data structure if necessary
      setSizeList(sizes);
  ...
  useEffect(() => {
    const filtered = sizeList.filter((size) =>
      (size.sizeName || "").toLowerCase().includes(input.toLowerCase())
    );
  ...
  const currentSizes = entriesPerPage === 'all' ? filteredSizes : filteredSizes.slice(indexOfFirst, indexOfLast);
  const totalPages = entriesPerPage === 'all' ? 1 : Math.ceil(filteredSizes.length / entriesPerPage);
  ...
  return (
    <div className="flex flex-col gap-6 mt-2 bg-white rounded-lg">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl font-semibold text-gray-600">Size List</h2>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          ...
          >
            <FaPlus className="inline mr-1" /> Add New Size
          </button>
      ...
      {entriesPerPage !== 'all' && (
        <div className="flex flex-col sm:flex-row justify-between items-center px-3 py-3 text-l text-gray-400">
          <span>
            Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filteredSizes.length)} of{" "}
