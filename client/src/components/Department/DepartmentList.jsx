...
const DepartmentList = () => {
  ...
  useEffect(() => {
    if (data) {
      const departments = data.departmentData || data;
  ...
  useEffect(() => {
    ...
    if (input === '') {
      ...
      const filtered = departmentList.filter(dep =>
        (dep.departmentName || '').toLowerCase().includes(input.toLowerCase())
      );
      setFilteredDepartments(filtered);
    }
    setCurrentPage(1);
    if (filteredDepartments.length === 0 && input !== '') {
      toast.info("No departments found for this search.");
    }
  }, [input, departmentList, filteredDepartments.length]);
  ...
  const totalPages =
    entriesPerPage === 'all'
      ? 1
      : Math.ceil(filteredDepartments.length / entriesPerPage);
  ...
  const handleAddDepartmentSuccess = (newDepartment) => {
    ...
    setDepartmentList(prevDepartments => [newDepartment, ...prevDepartments]);
    setFilteredDepartments(prevDepartments => [newDepartment, ...prevDepartments]);
    setInput(''); // Clear the search input after adding
  ...
  return (
    <div className="flex flex-col gap-6 mt-2 bg-white rounded-lg">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl font-semibold text-gray-600">Department List</h2>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          ...
          >
            <FaPlus className="inline mr-1" /> Add New Department
          </button>
          <button
            className="bg-violet-500 hover:bg-violet-600 text-white px-4 py-2 rounded-md text-sm"
      ...
      <AddDepartment
        ...
        handleAddDepartmentSuccess={handleAddDepartmentSuccess}
      />
            ...
            {currentDepartments.length > 0 ? (
              currentDepartments.map((item) => (
                <DepartmentItem
                  ...
                  isChecked={selectedDepartments.includes(item._id)}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center px-3 py-3 text-l text-gray-400">
        <span>
          ...
          {Math.min(indexOfLast, filteredDepartments.length)} of{' '}
