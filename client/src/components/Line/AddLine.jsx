...
const AddLine = ({ isOpen, setIsOpen, setLineList, userId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${base_url}/api/lines`,
        { userId, lineName: lineName.trim() },
        { withCredentials: true }
      );
      
      // Update the line list by adding the newly created line
      setLineList((prev) => [
        ...prev,
        { lineName: lineName.trim(), _id: data?.id, userId },  // Add the new line object
      ]);

      toast.success("Line added successfully!");
      setIsOpen(false); // Close the modal after success
      setLineName(''); // Clear the input field
    } catch (error) {
      console.error( error);
