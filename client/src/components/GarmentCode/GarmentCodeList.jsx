...
import AddGarment from './AddGarmentCode'; // Assuming you have an AddSection component
...
const GarmentList = () => {
  ...
  const filteredGarment = Garment.filter((Garment) =>
    Garment.name.toLowerCase().includes(input.toLowerCase())
  );

  const isAll = entriesPerPage === 'all';
  const itemsPerPage = isAll ? filteredGarment.length : parseInt(entriesPerPage);
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentGarment = filteredGarment.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = isAll ? 1 : Math.ceil(filteredGarment.length / itemsPerPage);
  ...
  return (
    <div className="flex flex-col gap-6 mt-2 bg-white rounded-lg">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl p-2 font-semibold text-gray-600">Garment Code List</h2>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-4">
        ...
        <div className="flex flex-wrap gap-2 justify-end">
          <button
            ...
            onClick={handleAddGarment} // Corrected handler
          >
            <FaPlus className="inline mr-1" /> Add New Garment
          </button>
        ...
        <table className="min-w-full text-sm text-left border border-gray-200">
          <thead className="bg-gray-50 text-xs text-gray-500 uppercase">
            <tr>
              ...
              <th className="p-4">Garment Code</th>
          ...
          <tbody>
            {currentGarment.length > 0 ? (
              currentGarment.map((Tidno, index) => (
                <tr key={index}>
                  <td className="p-4">
                    <input type="checkbox" />
                  </td>
      ...
      {!isAll && (
        <div className="flex flex-col sm:flex-row justify-between items-center px-4 py-3 text-l text-gray-400">
          <span>
            ...
            {Math.min(indexOfLastItem, filteredGarment.length)} of{' '}
            {filteredGarment.length} entries
          </span>
